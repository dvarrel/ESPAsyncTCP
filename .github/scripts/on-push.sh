...
if [ "$BUILD_PIO" -eq 0 ]; then
	...
	if [ ! "$OS_IS_WINDOWS" == "1" ]; then
		echo "Installing ESPAsyncWebSrv ..."
		git clone https://github.com/me-no-dev/ESPAsyncWebSrv "$ARDUINO_USR_PATH/libraries/ESPAsyncWebSrv" > /dev/null 2>&1

		echo "Installing ArduinoJson ..."
		git clone https://github.com/bblanchon/ArduinoJson "$ARDUINO_USR_PATH/libraries/ArduinoJson" > /dev/null 2>&1

		build_sketches "$FQBN" "$ARDUINO_USR_PATH/libraries/ESPAsyncWebSrv/examples"
	fi
else
	# PlatformIO Test
	...
	if [[ "$OSTYPE" != "cygwin" ]] && [[ "$OSTYPE" != "msys" ]] && [[ "$OSTYPE" != "win32" ]]; then
		echo "Installing ESPAsyncWebSrv ..."
		python -m platformio lib -g install https://github.com/me-no-dev/ESPAsyncWebSrv.git > /dev/null 2>&1
		git clone https://github.com/me-no-dev/ESPAsyncWebSrv "$HOME/ESPAsyncWebSrv" > /dev/null 2>&1

		echo "Installing ArduinoJson ..."
		python -m platformio lib -g install https://github.com/bblanchon/ArduinoJson.git > /dev/null 2>&1

		build_pio_sketches "$BOARD" "$HOME/ESPAsyncWebSrv/examples"
	fi
fi
